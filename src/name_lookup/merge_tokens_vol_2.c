...
/*   Updated: 2024/10/21 21:04:36 by aamirkha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	return (find_addr(quoted_tokens, token) || (!is_self_mergeable(token) && !is_parenthesis(token->val) && !string_equal(token->val, " ")));
}

bool	__small_condition__(t_listnode *token, t_shell *shell)
{
	return (!find_addr(shell->quoted_tokens, token) && (string_equal(token->val, "<<") || string_equal(token->val, ">>") || is_parenthesis(token->val)));
}
...
bool	__scary_condition__(t_listnode *token, t_listnode *next, t_shell *shell)
{
	return ((find_addr(shell->quoted_tokens, token)
		...
		|| (is_mergeable(token, shell->quoted_tokens)
			&& is_mergeable(next, shell->quoted_tokens)));
}
